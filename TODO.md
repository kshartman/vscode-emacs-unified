...
Cherry-picked into 1.3.1 (keybinding `when`-clause changes — manually verified in Windows VS Code, since the automated suite can't cover keybinding behavior):

- [x] **#2871** exclude `inlineEditIsVisible` from `tabToTabStop` when clause (Tab accepts Copilot NES instead of tab-to-tab-stop)
- [x] **#2869** clipboard shortcuts (C-w/M-w/C-y) in find/search widgets and webviews
