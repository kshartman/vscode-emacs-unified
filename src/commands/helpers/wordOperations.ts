...
// `_` is a word constituent in JS regexp `\w` but a separator in Emacs's subword
// handling, so the character classes use `[\W_]` to treat underscores as separators (#2816).
...
// `_` is treated as a separator (via `[\W_]`) to match Emacs's subword handling (#2816).
function findPreviousSubwordStartInternal(
  ...
  // At the start of a line, wrap back to the previous non-empty line, seeding the
  // effective cursor at its end so the all-caps split below fires correctly.
